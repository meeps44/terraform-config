...
# get destination tcp-port from input args
destination_port=$1 
flow_labels=(230 44)
...
do
    for flow_label in "${flow_labels[@]}"; do
        ...
        python3 ~/git/terraform-config/python-scripts/text-to-json.py ../bash-scripts/${filename} $HOSTNAME ${destination_port} ${host_ip} ${flow_label}
